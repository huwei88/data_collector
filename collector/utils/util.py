...
opts = [cfg.StrOpt('md5_cache_file',
        default='.md5_cache',
...
def data_changed(name, id):
    ...
    if not os.path.exists(CONF.get('md5_cache_file')):
        with open(CONF.get('md5_cache_file'), 'w') as f:
            cf.add_section(CONF.get('data_md5'))
            cf.write(f)
        return True
    cf.read(CONF.get('md5_cache_file'))
    try:
        cf.get(CONF.get('data_md5'), id)
        return False
    except ConfigParser.NoOptionError:
        cf.set(CONF.get('data_md5'), name, id)
        with open(CONF.get('md5_cache_file'), 'w') as f:
            cf.write(f)
        return True
...
def send_data(fun):
    def wrapper(**kwargs):
        ...
        for data_name, content in data_names, contents:
            ID = hashlib.md5().update(contents).hexdigest()
            if data_changed(data_name, ID):
                con = content
            else:
                con = ''
            wrapped_data = {'name': data_name, 'date': get_local_date(),
                         'content': con, 'id': ID, 'hostname': socket.gethostname(),
                         'output_module': 'default'}
            rpc.cast(topic=CONF.get('server_topic'), ctxt={}, method='store_data', **wrapped_data)
    return wrapper
